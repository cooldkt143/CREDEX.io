...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const retryRes = await fetch(`/api/auth/profile/${user.uid}`);
          if (retryRes.ok) {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (res.ok) {
        const updatedRes = await fetch(`/api/auth/profile/${user.uid}`);
        if (updatedRes.ok) {
  ...
  const handleResumeUpload = async (e) => {
    ...
    try {
      setIsUploading(true);
      const res = await fetch(`/api/auth/profile/${user.uid}/upload-resume`, {
        method: "POST",
        body: formData,
      });
                ...
                {dbProfile?.resume ? (
                  <div className="bg-black/30 border border-teal-400/20 rounded-xl p-4 flex items-center justify-between gap-4 mt-2">
                    ...
                    <div className="flex gap-2">
                      <a
                        href={`/${dbProfile.resume.file_path}`}
                        target="_blank"
                        rel="noreferrer"
